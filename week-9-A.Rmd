...
```
  ...
  Not accurate because there are no data beyond 90 km.
  ...
  Approximately a 0.1 (10%) difference in poverty level.
...
```{r sc1b}
fit_obj <- stan_glm(poverty ~ distance,
         data = week_9,
         refresh = 0)
...
$$reg_{chg} = \beta_0 + \beta_1x_{treat} + \beta_2x_{pov, i} + \beta_3x_{dist, i}  + \beta_4x_{pop, i} + \epsilon_i$$
...
fit_2 <- stan_glm(reg_chg ~ treatment + poverty + distance + pop_density,
         data = week_9,
         refresh = 0) 
